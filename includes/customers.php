...
class B4RCP_Customers {
	...
	public function add_customer_public_fields() {
		...
		?>
			<p>
				<label for="b4rcp_billing_name"><?php _e( 'Billing name', 'cps-billingo-for-rcp' ); ?></label>
				<input name="b4rcp_billing_name" id="b4rcp_billing_name" type="text" value="<?php echo esc_attr( $billing_name ); ?>"/>
			</p>
			<p>
				<label for="b4rcp_billing_address_countrycode"><?php _e( 'Billing address', 'cps-billingo-for-rcp' ); ?></label>
			...
			<p>
				<input
					...
					placeholder="<?php _e( 'Zip code', 'cps-billingo-for-rcp' ); ?>"
					type="text"
			...
			<p>
				<input
					...
					placeholder="<?php _e( 'City', 'cps-billingo-for-rcp' ); ?>"
					type="text"
			...
			<p>
				<input
					...
					placeholder="<?php _e( 'Street & Apartment', 'cps-billingo-for-rcp' ); ?>"
					type="text"
					value="<?php echo esc_attr( $billing_address_street ); ?>"
				/>
			</p>
			<p>
				<label for="b4rcp_tax_number"><?php _e( 'Tax number', 'cps-billingo-for-rcp' ); ?>
					<span class="field-optional"> (<?php _e( 'Optional', 'cps-billingo-for-rcp' ); ?>)</span>

				</label>
				<input
					name="b4rcp_tax_number"
					id="b4rcp_tax_number"
					placeholder="<?php _e( 'Only for companies', 'cps-billingo-for-rcp' ); ?>"
					type="text"
	...
	public function add_customer_admin_fields( $user_id = 0 ) {
		...
		?>
			...
			<tr>
				<th scope="row" class="row-title">
					<label for="b4rcp_billing_address"><?php _e( 'Billing address', 'cps-billingo-for-rcp' ); ?></label>
				</th>
				<td>
					<p>
						<?php echo b4rcp_get_country_select_field( $billing_address_countrycode ? $billing_address_countrycode : 'HU', 'b4rcp_billing_address_countrycode', 'b4rcp_billing_address_countrycode' ); ?>
			...
			<tr>
				<th scope="row" class="row-title">
					<label for="b4rcp_tax_number"><?php _e( 'Tax number', 'cps-billingo-for-rcp' ); ?></label>
				</th>
				<td>
			...
			<tr>
				<th scope="row" class="row-title">
					<label for="b4rcp_partner_id"><?php _e( 'Billingo partner id', 'cps-billingo-for-rcp' ); ?></label>
				</th>
				<td>
			...
			<tr>
				<td colspan="2">
					<h4><?php _e( 'Billingo partner connection log', 'cps-billingo-for-rcp' ); ?></h4>
	...
	public function validate_customer_fields_on_register( $posted ) {
		...
		if( empty( $posted['b4rcp_billing_name'] ) ) {
			rcp_errors()->add( 'invalid_billing_name', __( 'Please enter a billing name', 'cps-billingo-for-rcp' ), 'register' );
		}

		if( empty( $posted['b4rcp_billing_address_countrycode'] ) ) {
			rcp_errors()->add( 'invalid_billing_address_countrycode', __( 'Please enter a country code', 'cps-billingo-for-rcp' ), 'register' );
		} else if ( strlen($posted['b4rcp_billing_address_countrycode']) != 2  ) {
			rcp_errors()->add( 'invalid_billing_address_countrycode', __( 'Country code must be 2 characters. E.g. HU, DE, AT etc.', 'cps-billingo-for-rcp' ), 'register' );
		}

		if( empty( $posted['b4rcp_billing_address_zipcode'] ) ) {
			rcp_errors()->add( 'invalid_billing_address_zipcode', __( 'Please enter a zip code', 'cps-billingo-for-rcp' ), 'register' );
		}

		if( empty( $posted['b4rcp_billing_address_city'] ) ) {
			rcp_errors()->add( 'invalid_billing_address_city', __( 'Please enter a city', 'cps-billingo-for-rcp' ), 'register' );
		} else if ( strlen($posted['b4rcp_billing_address_city']) < 2  ) {
			rcp_errors()->add( 'invalid_billing_address_city', __( 'City must be at least 2 characters', 'cps-billingo-for-rcp' ), 'register' );
		}

		if( empty( $posted['b4rcp_billing_address_street'] ) ) {
			rcp_errors()->add( 'invalid_billing_address_street', __( 'Please enter street & apartment', 'cps-billingo-for-rcp' ), 'register' );
		} else if ( strlen($posted['b4rcp_billing_address_street']) < 2  ) {
			rcp_errors()->add( 'invalid_billing_address_street', __( 'Street & apartment must be at least 2 characters', 'cps-billingo-for-rcp' ), 'register' );
		}
	...
	public function get_partner_connection_log_table( $user_id ) {
		...
		if( is_array($log) ) {
			$o .= '<table>';
				$o .= '<tr>';
						$o .= '<th>'. __( 'Date', 'cps-billingo-for-rcp' ) .'</th>';
						$o .= '<th>'. __( 'Message', 'cps-billingo-for-rcp' ) .'</th>';
						$o .= '<th>'. __( 'Partner id', 'cps-billingo-for-rcp' ) .'</th>';
				$o .= '</tr>';
			...
			$o = __( 'There is no log to display', 'cps-billingo-for-rcp' );
		}
