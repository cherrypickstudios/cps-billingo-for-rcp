...
class B4RCP_Settings {
  ...
  public function plugin_settings_page_submit() {
    ...
    if( $res['response']['code'] != 200 ) {
      add_action( 'admin_notices', function() { ?>
        <div class="notice notice-error">
          <p><?php _e( 'Somthing\'s wrong with your api key.', 'billingo-for-rcp' ); ?></p>
        </div> <?php
      });
      update_option( 'b4rcp_is_connected', false );
      return false;
    ...
    if( $block_key === false ) {
      add_action( 'admin_notices', function() { ?>
        <div class="notice notice-error">
          <p><?php _e( 'The blockid you set up is wrong or there is no block with this id.', 'billingo-for-rcp' ); ?></p>
        </div> <?php
      });
      update_option( 'b4rcp_is_connected', false );
      return false;
  ...
  public function setup_section( $arguments ) {
    switch( $arguments['id'] ){
      case 'billingo_connection':
        _e('Generate your billingo api key <a href="https://app.billingo.hu/api-key" target="_blank">here</a> with reading and writing rule!', 'billingo-for-rcp' );
        break;
      case 'billingo_block_settings':
        _e('Choose whitch block you\'d like to generate invoices into and copy-paste its API ID from <a href="https://app.billingo.hu/document-block/list" target="_blank">here</a>!', 'billingo-for-rcp' );
        break;
    }
  }
